...
def main() -> None:
    ...
    output.append(section("9. COMPLETE SAFE SOURCE APPENDIX"))
    output.append(paragraph("This appendix is included so the compendium remains exhaustive when converted to a paginated PDF. It reproduces the text source of every included repository file after excluding secrets, local databases, cached market data, virtual environments, binary assets, generated build products, and this output file itself. It is a reference copy of the current working-tree source, not evidence of a production deployment."))
    for number, path in enumerate(files, 1):
        text = path.read_text(encoding="utf-8", errors="replace")
        appendix_text = "\n".join(line.rstrip() for line in text.splitlines())
        output.append(f"\n{'#' * 88}\n")
        output.append(f"SOURCE APPENDIX {number:03d}: {rel(path)}\n")
        output.append(f"{'#' * 88}\n")
        output.append(appendix_text + "\n")
